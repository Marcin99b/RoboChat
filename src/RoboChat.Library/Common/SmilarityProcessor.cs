...
{
    public class SmilarityProcessor
    {
        ...
        public double GetSimilarityRatio(String fullString1, String fullString2, out double wordsRatio, out double realWordsRatio)
        {
            double theResult = 0;

            fullString1 = fullString1.ToLowerInvariant();
            fullString2 = fullString2.ToLowerInvariant();

            var punctuationFirst = fullString1.Where(char.IsPunctuation).Distinct().ToArray();
            var Splitted1 = fullString1.Split().Select(x => x.Trim(punctuationFirst)).Distinct().ToArray();
            var punctuationSecond = fullString2.Where(char.IsPunctuation).Distinct().ToArray();
            var Splitted2 = fullString2.Split().Select(x => x.Trim(punctuationSecond)).Distinct().ToArray();
